...
function getDesiredInfraCount() {
  echo "Find desired Infra node count"
  desired_infra_count=$(ocm get /api/clusters_mgmt/v1/clusters/"$CLUSTER_ID"/node_pools | jq -r '[.items[] | select(.labels != null) | select(.labels."node-role.kubernetes.io/infra").replicas] | add')
...
function checkInfra() {
    ...
    while [ $TRY -le 3 ]; do # Attempts three times to migrate pods
        ...
        if [[ $FLAG_ERROR == "" ]]; then return 0; else TRY++; fi
    done
